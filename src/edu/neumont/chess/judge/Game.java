...
//	}
	...
	public void playGame() {
		synchronized( Judge.class ) {
			if( judgeListener == null ) {
				judgeListener = new JudgeEventAdapter() {
					...
					public void timeBetweenTurnsChanged(JudgeEvent e) {
						if( game != null )
							game.setTimeBetweenTurns( e.getSource().getTimeBetweenTurns() );
					}
				};
			...
			try {
				ProcessBuilder whiteBuilder = new ProcessBuilder("java", "-jar", white.getJar().getAbsolutePath(), "white");
				whiteProcess = whiteBuilder.start();
				//whiteProcess = Runtime.getRuntime().exec( "java -jar " + white.getJar().getAbsolutePath() + " white");
				ProcessBuilder blackBuilder = new ProcessBuilder("java", "-jar", black.getJar().getAbsolutePath(), "black");
				blackProcess = blackBuilder.start();
				//blackProcess = Runtime.getRuntime().exec( "java -jar " + black.getJar().getAbsolutePath() + " black");
				if( game != null ) {
				...
				mediator.createPlayerFromProcess(Team.White, whiteProcess, white.getName());
				mediator.createPlayerFromProcess(Team.Black, blackProcess, black.getName());
	...
	public void showGUI() {
		if( game != null ) {
			...
			if( gui == null ) {
				gui = new ChessGUI(game, white.getName() + " vs " + black.getName());
